...
def get_args():
    ...
    parser.add_argument('-s','--save', type=bool, default=False, help='Save Cat Jac DF. [Default: False]')
...
def welcome_txt():
    ...
    print('                                                      SummitHill Biotech, V1.01.01')
...
def main():
    ...
    if args.fast: fast_tag = '_FAST'
    else: fast_tag = ''
    os.makedirs(args.out_dir,exist_ok=True)
    for i, row in DF.iterrows():
        Name = f'{row.seq_a_name}-{row.seq_b_name}{fast_tag}'
        print('-'*50,f'\n> {i}/{DF.shape[0]} [{(time.time()-t0):.2f} sec] {Name}')
        # define seq - 
        sequence = f"{row.seq_a}{row.seq_b}"
        # run cat jac
        J, contact, tokens = get_categorical_jacobian(sequence, fast=args.fast)
        df = contact_to_dataframe(contact)
        # create and save figure:
        out_file = clean_string(Name)
        outpath = os.path.join(args.out_dir,f'{out_file}.png')
        figure_dict = {'Title':f"{Name}\n Conservation", 'outpath':outpath, 'seq0_len':len(row.seq_a), 'seq1_len':len(row.seq_b), 'labels':[row.seq_a_name, row.seq_b_name]}
        create_figure(df, tokens, **figure_dict)
        if args.save:
            df_outpath = os.path.join(args.out_dir,f'{out_file}.csv')
            df.to_csv(df_outpath,index=None)
